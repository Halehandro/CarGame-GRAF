class Game{
    ...
    carGUIHandler( part ){
        //0 - Tijelo, 1 - Aerial, 2 - Motor, 3 - Auspuh, 4 - Kotacif
        this.sfx.click.play();
        this.carGUI[part]++;
    ...
    startGame(){
        this.sfx.click.play();
        const parts = ["Tijelo", "Aerial", "motor", "auspuh", "neke kotače"];
        let index = 0;
        let configured = true;
        this.carGUI.forEach(function(item){
           if (item==0){
               showMessage(`Molim odababerite ${parts[index]}`);
               configured = false;
           }
            index++;
        });
...
class JoyStick{
	...
	tap(evt){
		...
		// dohvaćanje pozicije kursora na početku
...
class Preloader{
	constructor(options){
		...
		if (options.onprogress==undefined){
			...
			barBase.style.background = 'orange';
